...
{
        KASSERT(vn);
        //dbg(DBG_VNREF, "vref: 0x%p, 0x%p ino %ld at  %d, nrespages=%d\n",
        //   vn, vn->vn_fs, (long)vn->vn_vno, vn->vn_refcount, vn->vn_nrespages);
        KASSERT(0 < vn->vn_refcount);
        vn->vn_refcount++;
        if(vn->vn_vno == 41){dbg(DBG_TEMP, "incrementing to vnode: %d\n", vn->vn_refcount);}
...
if(vn->vn_vno == 41){dbg(DBG_TEMP, "decrementing to vnode: %d\n", vn->vn_refcount -1);}
...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
    ...
    // check if the file is a byte mode
