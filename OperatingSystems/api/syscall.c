...
sys_read(read_args_t *arg)
{
    ...
    read_args_t temp;
    if(copy_from_user(&temp, arg, sizeof(read_args_t)) < 0){
        curthr->kt_errno = EINVAL;
        return -1;
    }
    void *buffer = page_alloc();
    int bytesRead = do_read(temp.fd, buffer, temp.nbytes);
    if(bytesRead < 0){
            curthr->kt_errno = -bytesRead;
            return -1;
    }
    if(copy_to_user(arg->buf, (const void *)buffer, bytesRead) <0){
        curthr->kt_errno = EINVAL;
        return -1;
    }
    page_free(buffer);
...
{ 
    ...
    if(copy_from_user(&temp, arg, sizeof(write_args_t)) < 0){
        curthr->kt_errno = EINVAL;
        return -1;
    }
    //void *buffer = page_alloc();
    ...
    int bytesWritten = do_write(temp.fd, temp.buf, temp.nbytes);
    
    if(bytesWritten < 0){
            curthr->kt_errno = -bytesWritten;
            return -1;
    }
    /*
    if(copy_to_user(arg->buf, (const void *)buffer, sizeof(buffer)) < 0){
        curthr->kt_errno = EINVAL;
        return -1;
    }
    page_free(buffer);
...
sys_getdents(getdents_args_t *arg)
{
    ...
    getdents_args_t temp;
    if(copy_from_user(&temp, arg, sizeof(getdents_args_t)) < 0){
        curthr->kt_errno = EINVAL;
        return -1;
    }
    size_t counter = 0;
    ...
    int returnVal;
    //Need to double check with "read getdent_args_t->count bytes", probably need some changes in the condition
    while(counter < arg->count){
            
            returnVal = do_getdent(temp.fd, (dirent_t*)((char*)temp.dirp+ counter));
            if(returnVal == 0){
                break;
            }
   
            if(returnVal < 0){
                    curthr->kt_errno = -returnVal;
                    return -1;
            }
    ...
    //*temp.dirp = *buffer;
    if(copy_to_user(arg->dirp, temp.dirp, sizeof(dirent_t)) < 0){
