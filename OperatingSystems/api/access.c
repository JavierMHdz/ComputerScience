...
{       
    dbg(DBG_TEMP, "curproc: %d, vaddr: 0x%x\n", curproc->p_pid, (uintptr_t)vaddr);
    //NOT_YET_IMPLEMENTED("VM: addr_perm");
    vmmap_t* map = p->p_vmmap;
    uint32_t vfn = ADDR_TO_PN(vaddr);
    
    vmarea_t* curr_vma = vmmap_lookup(map, vfn);
    KASSERT(curr_vma != NULL);
/*
    if(curr_vma == NULL){
        return 0;
    }
    */
    if(perm & PROT_READ && !(curr_vma->vma_prot & PROT_READ)){
        return 0;
    }
    if(perm & PROT_WRITE && !(curr_vma->vma_prot & PROT_WRITE)){
        return 0;
    }
    if(perm & PROT_EXEC && !(curr_vma->vma_prot & PROT_EXEC)){
        return 0;
    }
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
    ...
    dbg(DBG_TEMP, "curproc: %d, avaddr: 0x%x\n", curproc->p_pid, (uintptr_t)avaddr);
    uintptr_t startaddr = (uintptr_t)avaddr;
    
    if(avaddr == NULL){
        return 0;
    }
    //uintptr_t lopage = ADDR_TO_PN(avaddr);
    //uintptr_t endpage = ADDR_TO_PN((uintptr_t)avaddr + len);
    if(USER_MEM_LOW > (uintptr_t)avaddr || USER_MEM_HIGH < (uintptr_t)avaddr + len) {
        return 0;
    }
    
    uint32_t searchedvpn = NULL;
    uint32_t currvpn;

    while(startaddr <= (uintptr_t)avaddr + len){

        currvpn = ADDR_TO_PN(startaddr);
        if(searchedvpn == NULL){
            if(!addr_perm(p, PN_TO_ADDR(currvpn), perm)){
                return 0;
            }
            searchedvpn = currvpn;
        }

        if(currvpn != searchedvpn){
